...
class Solution:
    def minimumDeletions(self, nums: list[int]) -> int:
        ...
        # 2. Order the indices so i is always the smaller index
        i, j = min(min_idx, max_idx), max(min_idx, max_idx)
        
        # 3. Calculate deletions for the 3 available strategies
        del_both_sides = (i + 1) + (n - j)
        del_from_front = j + 1
        del_from_back = n - i
        
        # 4. Return the minimum of the three strategies
        return min(del_both_sides, del_from_front, del_from_back)
